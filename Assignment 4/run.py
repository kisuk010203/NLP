...
def train(args: Dict):
    ...
    model = NMT(
        embed_size=1024, hidden_size=1024, dropout_rate=float(args["--dropout"]), vocab=vocab
    )
...
def decode(args: Dict[str, str]):
    ...
    if args["TEST_TARGET_FILE"]:
        print(
            "load test target sentences from [{}]".format(args["TEST_TARGET_FILE"]), file=sys.stderr
        )
